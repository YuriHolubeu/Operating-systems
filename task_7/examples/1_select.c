/* Программа демонстрирует использование вызова select() для мультиплексирования
...
int main()
{
    fd_set rfds; //специальная структурка, в которую записываются файловые дескрипторы
    // специальным макросом FD_SET
    ...
    if (retval) {
        ...
        printf("Статус FD_ISSET: %d\n", FD_ISSET(0, &rfds)); //вернет истинное значение в
        //случае успеха
